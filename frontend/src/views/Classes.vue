<template>
  <div class="pane">
    ...
    <div class="days">
      <template v-for="g in byDay" :key="g.date">
        ...
        <div v-for="c in g.list" :key="c.id" class="c-row">
          ...
          <span class="c-area">{{ c.areaName || '未排区' }}</span>
          <span class="c-time">{{ c.startTime || '待定' }}<template v-if="c.endTime">-{{ c.endTime }}</template></span>
          <div class="c-seat">
    ...
    <el-dialog v-model="dialog" title="开一门团课" width="440px">
      ...
      <div class="fr"><label>上课时间</label><el-input v-model="form.startTime" placeholder="19:00" /></div>
      <div class="fr"><label>下课时间</label><el-input v-model="form.endTime" placeholder="19:45" /></div>
      <div class="fr"><label>训练区</label>
        <el-select v-model="form.areaId" placeholder="上课占用的区" clearable style="flex:1">
...
async function load() {
  ...
  areas.value = (await areaApi.list()).filter((a) => a.areaState === '开放')
}
function openNew() {
  form.value = {}
  dialog.value = true
...
.c-row { display: grid; grid-template-columns: 96px 1.2fr 96px 96px 110px 160px 90px 80px; gap: 8px;
